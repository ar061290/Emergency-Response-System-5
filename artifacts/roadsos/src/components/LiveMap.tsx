...
import { Bus, Heart, Ambulance, Building2 } from "lucide-react";
import { renderToString } from "react-dom/server";
import type { SchoolBus } from "@workspace/api-client-react";

/* ── Custom icon helpers ── */
function makeIcon(color: string, content: React.ReactNode) {
  const svgString = renderToString(
    <div style={{ width: 28, height: 28, borderRadius: 999, background: color, border: "2px solid #fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 6px rgba(0,0,0,0.4)" }}>
      {content}
    </div>
  );
  return L.divIcon({
  ...
  return (
    <div className="relative rounded-b-lg overflow-hidden" style={{ height: 380 }}>
      ...
      >
            ...
            <Popup className="!text-foreground">
              <div className="space-y-1">
                ...
                {bus.currentLocationName && (
                  <div className="text-xs text-muted-foreground">{bus.currentLocationName}</div>
                )}
        ...
        {/* Route line from child to hospital */}
        ...
        {/* Route line from ambulance to child */}
      ...
      <div className="absolute bottom-3 left-3 z-[500] bg-card/90 border border-border rounded-lg px-3 py-2 space-y-1 shadow-lg backdrop-blur">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-slate-300">Ambulance</span>
        </div>
        <div className="flex items-center gap-1.5">
